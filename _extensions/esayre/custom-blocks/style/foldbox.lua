...
blockStart = function (tt, fmt)
  ...
  elseif fmt =="tex" then
    ...
    return('\\begin{'..texEnv..'}{'..tt.type..'}{\\'..tt.type..'faicon{}}{'..tt.typlabelTag..tt.title..'}\n'..
  ...
  local extractStyleFromMeta = function (fmt)
    ...
    if classDefs ~= nil then
      for cls, options in pairs(classDefs) do
        -- pout("Class options in ClassDefs:"); pout(options) 
        -- faicon is here...
        if options.colors then
          ...
          elseif fmt == "pdf" then
           ...
           -- faicon is here too.  
           if options.faicon then table.insert(StyleCSSTeX, "\\newcommand{\\"..cls.."faicon}{\\fa"..options.faicon.."}\n\n") end
          end  
        end  
      end  
    end  
    ...
    if fmt == "pdf" then result="%%==== colors and icons from yaml ===%\n"..result.."%=============%\n" end
    return(result)
  end
